...
def userpage(request):
    obj = ddos_dataset.objects.all().order_by('-id')
...
def add_data(request):
    ...
    if request.method == "POST":
        ...
        if len(attack1) > len(attack2) and len(attack1) > len(attack3) and len(attack1) > len(attack4) and len(
            ...
            ans = "Not Attacked Data"
        ddos_dataset.objects.create(ddos_data=txt,attack_result=ans)
    return render(request,'add_data.html',{"attack_result": ans})
...
def unlabeled_data(request):
    obj = ddos_dataset.objects.filter(attack_result='Not Attacked Data').order_by('-id')
